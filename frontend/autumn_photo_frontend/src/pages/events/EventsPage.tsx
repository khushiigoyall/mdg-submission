...
export default function EventsPage() {
 ...
 const fetchPhotos = async (id:any) => {
  ...
  return (
    <div className="h-screen w-screen overflow-y-auto bg-[#111010] text-[#e8e3dc] font-sans">
      <Navbar />
      <div className="relative z-10 px-6 md:px-10 lg:px-16 pt-20 pb-10">
        ...
        <div className="w-full mb-12">
          <div className="relative flex items-center">
            <div className="absolute left-4 text-[#7a7570]">
            ...
            <button
              ...
              className="absolute right-2 px-6 py-2 rounded-md bg-[#c9a96e] hover:bg-[#b0935d] text-[#111010] font-semibold transition-colors duration-200"
            >
              Search
            </button>
          </div>
                ...
                {events.map((ev) => (
                  ...
                  >
                    <div className="relative overflow-hidden h-56 shrink-0 bg-[#111010]">
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-white/[0.08] backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-medium text-white border border-white/[0.05]">
                        <Users className="w-3.5 h-3.5" />
                        <span>{ev.coordinators?.length || 0}</span>
                      </div>
                      
                      <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-[#c9a96e]/[0.15] backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-medium text-[#c9a96e] border border-[#c9a96e]/20">
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      <h2 className="text-2xl font-medium mb-2 text-[#e8e3dc] group-hover:text-[#c9a96e] transition-colors line-clamp-1">
                        {ev.name}
                      </h2>
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-white/[0.05] pt-4">
                        <div className="flex items-center text-sm text-[#c9a96e] font-medium">
        ...
        {(selectedEvent || searchMode === "photos") && (
          <div className="animate-fadeIn">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 p-6 rounded-lg bg-[#1a1917] border border-white/[0.05] backdrop-blur-sm">
              <div>
                <button
                  className="inline-flex items-center gap-2 text-[#7a7570] hover:text-[#c9a96e] transition-colors mb-3 group"
                  onClick={() => {
                    setSelectedEvent(null);
                    setPhotos([]);
                    setSearchMode("events");
                ...
                <h2 className="text-3xl md:text-4xl font-normal text-[#e8e3dc]" style={{ fontFamily: "'DM Serif Display', serif" }}>
                  {selectedEvent ? selectedEvent.name : `Search Results: "${searchQuery}"`}
                </h2>
                <p className="text-[#7a7570] mt-2">{photos.length} photos available</p>
              </div>
              <div className="flex items-center gap-2 p-1.5 rounded-lg bg-[#111010] border border-white/[0.05]">
                <button
                  onClick={() => setView("grid")}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-md font-medium transition-all duration-200 ${
                    view === "grid"
                      ? "bg-[#c9a96e] text-[#111010] shadow-[0_4px_15px_rgba(201,169,110,0.2)]"
                      : "text-[#7a7570] hover:text-[#e8e3dc] hover:bg-white/[0.05]"
                  }`}
                >
            ...
            {photos.length > 0 ? (
              <>
                  ...
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px]">
                    {photos.map((p) => (
                      <div
                        ...
                        className="group relative rounded-lg overflow-hidden bg-[#1a1917] border border-white/[0.06] hover:border-[#c9a96e]/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(201,169,110,0.12)] cursor-pointer"
                      >
                        <img src={getMediaUrl(p.thumbnail_file)}
                          alt="Event photo"
                          className="w-full h-64 object-cover group-hover:scale-105 transition-transform duration-700"
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/[0.6] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      </div>
                    ))}
                  </div>
                )}
                ...
                {view === "masonry" && (
                  <div className="columns-2 md:columns-3 lg:columns-4 gap-[20px]">
                    {photos.map((p) => (
                      <div
                        key={p.id}
                        onClick={() => { setOpenPhotoId(p.id); setOpenPhotoUrl(p.thumbnail_file); }}
                        className="group relative rounded-lg overflow-hidden bg-[#1a1917] border border-white/[0.06] hover:border-[#c9a96e]/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(201,169,110,0.12)] cursor-pointer mb-[20px] break-inside-avoid"
                      >
                        <img
                ...
                {view === "carousel" && (
                  <div className="relative">
                    <div className="flex overflow-x-auto gap-[20px] pb-6 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-[#c9a96e] scrollbar-track-[#1a1917]">
                      {photos.map((p) => (
                        <div
                          key={p.id}
                          onClick={() => { setOpenPhotoId(p.id); setOpenPhotoUrl(p.thumbnail_file); }}
                          className="group relative rounded-lg overflow-hidden bg-[#1a1917] border border-white/[0.06] hover:border-[#c9a96e]/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(201,169,110,0.12)] flex-shrink-0 snap-center"
                        >
                          <img
                            src={getMediaUrl(p.thumbnail_file)}
                            alt="Event photo"
                            className="h-80 w-auto object-cover group-hover:scale-105 transition-transform duration-700"
                          />
                          <div className="absolute inset-0 bg-gradient-to-t from-black/[0.6] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </div>
                      ))}
        ...
        {editModalOpen && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-[#1a1917] rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-white/[0.05] shadow-2xl">
              <div className="sticky top-0 bg-[#1a1917] border-b border-white/[0.05] p-5 flex items-center justify-between">
                <h2 className="text-2xl font-normal text-[#e8e3dc]" style={{ fontFamily: "'DM Serif Display', serif" }}>Edit Event</h2>
                <button onClick={closeEditModal} className="text-[#7a7570] hover:text-[#e8e3dc] text-3xl leading-none">&times;</button>
              </div>

              <form onSubmit={submitEdit} className="p-6 space-y-5 text-[#e8e3dc]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-2 text-[#7a7570]">Start Date & Time</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2 text-[#7a7570]">End Date & Time</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-[#7a7570]">Cover Photo</label>
